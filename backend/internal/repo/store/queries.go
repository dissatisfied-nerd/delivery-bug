...
const (
	...
	`
	insertStoreQuery = `
		INSERT INTO stores(name, first_name, middle_name, last_name, address_id) 
	...
	`
	checkLoginQuery = `
		SELECT login, password, store_id 
		FROM stores_loginform 
		WHERE login=$1
	`
	selectStoreQuery = `
		SELECT name, first_name, middle_name, last_name, address_id 
		FROM stores 
		WHERE id=$1
	`
	selectAddressQuery = `
