...
func (r *Repository) InsertStore(ctx context.Context, storeDTO dtos.StoreDTO, addressID string) (string, error) {
	...
	err := r.db.QueryRow(ctx, insertStoreQuery, storeDTO.Name, storeDTO.FirstName, storeDTO.MiddleName,
		storeDTO.LastName, addressID).Scan(&insertedStoreID)
	if err != nil {
...
func (r *Repository) SelectInfoByID(ctx context.Context, id string) (dtos.StoreInfo, error) {
	...
	err := r.db.QueryRow(ctx, selectStoreQuery, id).Scan(&info.Name, &info.FirstName, &info.MiddleName,
		&info.LastName, &addressID)
	if err != nil {
